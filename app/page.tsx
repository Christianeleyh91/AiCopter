...
const faq = [
  ...
  ['سرعت کواد چگونه تنظیم می‌شود؟', 'How are drone speeds configured?', 'پیش از اتصال به شبیه‌ساز، برنامه سرعت Patrol و سرعت نزدیک‌شدن به چهره را برحسب متر بر ثانیه دریافت و اعتبارسنجی می‌کند. در حالت گشت، سرعت اول اعمال می‌شود؛ هنگام رهگیری چهره، سرعت دوم جای آن را می‌گیرد. وقتی عرض کادر چهره به نسبت هدف برسد، سرعت جلو صفر می‌شود و فقط اصلاح yaw ادامه می‌یابد؛ با گم‌شدن هدف، گشت از سر گرفته می‌شود.', 'Before connecting to the simulator, the program requests and validates patrol speed and face-approach speed in meters per second. Patrol uses the first value, while face tracking uses the second. Once the face box reaches the target width ratio, forward speed becomes zero and only yaw correction continues; patrol resumes after the target is lost.'],
  ['ثبت هویت چگونه انجام می‌شود؟', 'How does identity enrollment work?', 'در نسخهٔ وب‌کم، چند عکس واضح و واقعاً متفاوت از یک فرد بررسی می‌شوند؛ هر عکس باید دقیقاً یک چهرهٔ قابل‌قبول داشته باشد. چهره هم‌تراز و به embedding تبدیل می‌شود و templateهای رمزگذاری‌شده زیر یک نام در پایگاه داده ذخیره می‌شوند. در نسخهٔ Unreal، برای هر کاراکتر پوشه‌ای جدا در known_people ساخته می‌شود و تصاویر همان کاراکتر از زاویه‌ها و نورهای مختلف داخل آن قرار می‌گیرند تا مدل LBPH در شروع برنامه آموزش ببیند.', 'In the webcam build, several clear and genuinely distinct photos are checked, and each must contain exactly one acceptable face. The face is aligned, converted into an embedding, and its encrypted templates are stored under one identity. In the Unreal build, each character has a separate known_people folder containing multiple views and lighting conditions used to train the LBPH model at startup.'],
  ...
  ['Scene و Depth چه تفاوتی دارند؟', 'What is the difference between Scene and Depth?', 'Scene همان تصویر رنگی دوربین است و برای آشکارسازی چهره، شناسایی هویت، رسم کادر و نمایش زنده استفاده می‌شود. DepthPerspective برای هر پیکسل فاصلهٔ تقریبی تا سطح دیده‌شده را برحسب متر فراهم می‌کند و ورودی منطق اجتناب از مانع است. دو تصویر در یک درخواست گرفته و سپس Depth با ابعاد Scene هم‌اندازه می‌شود تا تصمیم‌های بینایی و فاصله هماهنگ بمانند.', 'Scene is the camera’s color image used for face detection, identity recognition, overlays, and live display. DepthPerspective estimates the distance in meters from each pixel to the visible surface and drives obstacle avoidance. Both images are requested together, and Depth is resized to match Scene so perception and distance decisions stay spatially aligned.'],
  ...
  ['چه مدل‌ها و کتابخانه‌هایی استفاده شده‌اند؟', 'Which models and libraries are used?', 'YuNet کادر و پنج landmark چهره را پیدا می‌کند و SFace embedding هویتی می‌سازد. MediaPipe در نسخهٔ وب‌کم هندسهٔ دقیق‌تر صورت را برای Liveness فراهم می‌کند؛ OpenCV دریافت تصویر، هم‌ترازی، اجرای مدل‌ها و رابط تصویری را برعهده دارد و NumPy محاسبات آرایه‌ای را انجام می‌دهد. SQLite و Fernet برای ذخیرهٔ رمزگذاری‌شده و Cosys-AirSim برای ارتباط RPC با Unreal استفاده می‌شوند.', 'YuNet detects the face box and five landmarks, while SFace produces identity embeddings. In the webcam build, MediaPipe provides denser facial geometry for liveness; OpenCV handles capture, alignment, model inference, and visualization, while NumPy performs array computation. SQLite and Fernet support encrypted storage, and Cosys-AirSim provides RPC communication with Unreal.'],
  ['بعد از تشخیص چهره کواد چه می‌کند؟', 'What does the drone do after detecting a face?', 'اگر چند چهره دیده شوند، بزرگ‌ترین کادر به‌عنوان هدف عملیاتی انتخاب می‌شود. اختلاف مرکز چهره با مرکز تصویر به نرخ yaw تبدیل می‌شود تا کواد رو به هدف قرار بگیرد و تا رسیدن عرض چهره به نسبت هدف، با سرعت تنظیم‌شده نزدیک شود. سپس حرکت جلو صفر می‌شود؛ اجتناب از مانع در تمام مراحل اولویت بالاتری دارد و پس از گم‌شدن چهره، حالت Patrol دوباره فعال می‌شود.', 'If several faces are visible, the largest box becomes the operational target. Horizontal error between the face and image center is converted into yaw rate, while the quadrotor approaches at the configured speed until the face reaches the target width ratio. Forward motion then stops; obstacle avoidance always has higher priority, and Patrol resumes after the face is lost.'],
  ...
  ['چگونه سیستم را ارزیابی می‌کنید؟', 'How is the system evaluated?', 'دادهٔ آزمون باید از تصاویر ثبت هویت جدا باشد و افراد شناخته‌شده و ناشناس را در نور، فاصله و زاویه‌های مختلف پوشش دهد. معیارهای اصلی شامل False Accept Rate، False Reject Rate، نرخ Unknown، پایداری رأی زمانی، latency و FPS هستند. برای بخش پرواز نیز فاصلهٔ توقف، خطای مرکزکردن هدف، زمان بازیابی Patrol و درصد موفقیت اجتناب از مانع در سناریوهای تکرارشونده اندازه‌گیری می‌شود.', 'Evaluation data must be separate from enrollment and include both known and unknown identities across varied lighting, distance, and pose. Core metrics include false-accept rate, false-reject rate, Unknown rate, temporal stability, latency, and FPS. Flight evaluation also measures stopping distance, target-centering error, Patrol recovery time, and obstacle-avoidance success over repeatable scenarios.'],
];
